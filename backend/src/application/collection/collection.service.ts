...
import type { Collection, CollectionResponse } from '../../domain/collection/index.js';
...
export class CollectionService {
    ...
    async updateCollection(id: string, data: Partial<Collection>): Promise<CollectionResponse> {
        if (data.slug) {
            const existing = await this.repository.findBySlug(data.slug);
