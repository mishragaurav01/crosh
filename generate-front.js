...
// 3. Admin Pages
const adminPages = ['categories', 'collections', 'products', 'variants', 'inventory', 'pricing', 'images'];
adminPages.forEach(m => {
    save(\`frontend/src/app/(admin)/admin/${m}/page.tsx\`, \`'use client';
import { useQuery } from '@tanstack/react-query';
import axios from 'axios';

export default function Admin${m.charAt(0).toUpperCase() + m.slice(1)}Page() {
  const { data, isLoading } = useQuery({
    queryKey: ['admin-catalog-${m}'],
    queryFn: async () => (await axios.get('http://localhost:3000/api/v1/${m}')).data.data
  });

  return (
    <div className="p-8 bg-white min-h-screen">
      <div className="flex justify-between items-center mb-8 border-b pb-4">
        <h1 className="text-2xl font-bold capitalize">Manage ${m}</h1>
        <button className="bg-black text-white px-4 py-2 rounded text-sm font-bold shadow hover:bg-gray-800">Create New</button>
      </div>
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th className="p-3 capitalize">Name / Ref</th>
          ...
          <tbody>
            {(data && data.length > 0) ? data.map((item: any) => (
              <tr key={item.id || item._id} className="border-b hover:bg-gray-50 transition">
                <td className="p-3 font-mono text-xs text-gray-500">{item.id || item._id}</td>
                <td className="p-3 font-semibold">{item.name || item.sku || 'Item'}</td>
              ...
              <tr><td colSpan={3} className="p-6 text-center text-gray-500">No ${m} found. Start creating!</td></tr>
            )}
          </tbody>
        </table>
      )}
...
save('frontend/src/tests/catalog-ui.spec.tsx', \`import { describe, it, expect, vi } from 'vitest';
